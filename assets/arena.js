...
const parseBlocks = (data) => {
  let blocks = ['audioEmbed', 'audioFile', 'image', 'link', 'pdf', 'text', 'videoEmbed', 'videoFile'];
...
const renderBlock = (block, type) => {
  ...
  let element = ['title', 'image', 'embed', 'audio', 'video', 'link', 'linkTitle', 'content', 'description', 'type'];
  ...
  if (element.image) {
    block.image ? (element.image.src = block.image.display.url) : element.image.remove();
    console.log(block.image.display.url);
    // const item = $('div').attr('class', 'item').append($('img').attr('src', block.image.display.url));
